...
var _hour_text = string_format(floor(_hour), 2, 0) + ":" + string_format(floor((_hour - floor(_hour)) * 60), 2, 0);
...
var _frogs = 0;
var _thunder = 0;
if (_audio_last == "INSECT") _insects = 1;
if (_audio_last == "FROG") _frogs = 1;
if (_audio_last == "THUNDER") _thunder = 1;

var _box_h = 15 * _line + 20;
draw_set_alpha(0.90);
draw_set_color(c_black);
draw_rectangle(_x - 8, _y - 8, min(_w - 12, _x + 330), min(_h - 12, _y + _box_h), false);
draw_set_alpha(1);
...
draw_set_color(c_white);
draw_text(_x, min(_h - 18, _y + _box_h - 2), "F5 DEBUG  |  A = evento ambiental");

...
// Debug: F1 pausa, F2/F3 avancam uma hora, F4 x1/x12, F5 overlay.
if (time_debug)
{
...
// ====================================================
// LUA: uma escolha por noite, sem trocar durante o percurso.
// ====================================================
// A noite comeca em 19:00. Ao entrar nela, sorteamos a lua.
if (time_hours_now >= time_night_start && !moon_night_selected)
{
    select_moon_for_night();
}

// Ao sair da noite, liberamos a proxima selecao.
...
// Se a lua vermelha apareceu, conta como evento e zera a sequencia.
if (moon_was_red && time_hours_now >= time_night_start)
{
    // A flag fica registrada globalmente; a contagem e tratada uma vez por noite.
}

// Contagem das noites: ao chegar novamente na madrugada do dia seguinte,
// registra a noite que terminou. Lua vermelha reinicia a sequencia.
if (time_hours_now < time_dawn_start && time_prev_period == "MADRUGADA" && !moon_night_selected)
{
    // Mantido sem incremento repetitivo; a contagem efetiva ocorre na transicao de noite.
}

// Detecta entrada na madrugada para contabilizar a noite encerrada.
...
// Estado publico do Environment System.
...
// Escurecimento com curvas continuas. Nao salta de dusk para noite.
...
else if (time_hours_now >= H_DAWN && time_hours_now < H_MORNING)
{
    night_factor = 1 - smooth01((time_hours_now - H_DAWN) / (H_MORNING - H_DAWN));
}
else
{
    night_factor = 0;
...
// Estrelas: entram e saem gradualmente.
...
// Publica fatores para futuros sistemas.
